...
prt.code as salon_partner_code, prt."name" as salon_partner_name
from  salons as sln 
left join salon_types as slt on sln.salon_type_id = slt.id
left join regions_salons as rgs on sln.id = rgs.salon_id
left join region_partners as rgnp on rgs.region_id = rgnp.region_id
left join partners as prt on rgnp.partner_id = prt.id
...
to_char(sme.started_at::timestamp at time zone 'UTC','DD') as Day, 
to_char(sme.started_at::timestamp at time zone 'UTC','MM')as Month,
...
sln.salon_partner_code, sln.salon_partner_name , sme.description
...
where to_char(sme.started_at,'YYYY') in ('2017') and brn."name" is not null and smrkt."name" = 'Brand Day'
order by sme.started_at, sme.id
